...
class ComponentList extends Component {
	...
	getComponentNameCell = (props) => {
		...
		let componentStyle = [this.styles.component, this.styles.row];
		if (!entry.isMounted) {
			componentStyle.push(this.styles.unmountedComponent);
		}
		if (this.props.selectedComponentId === row.id) {
		...
		componentStyle = Object.assign.apply(this, componentStyle);
		return (
			<Cell
				{...props}
	...
	getRenderCountCell = (props) => {
		...
		return (
			<Cell>
				<div style={this.styles.renderCount}>{row.renderCount}</div>
			</Cell>
		)
	};
	...
	getWarningCountCell = (props) => {
		const row = this.state.componentTableData[props.rowIndex];
		const tooltip = (
			<Tooltip id='unnecessaryRendersTooltip'>{row.warningCount} unnecessary rerenders
				prevented</Tooltip>);
		let warningCount = '';
		if (row.warningCount) {
			warningCount = (
				<div>
					<OverlayTrigger
						...
						overlay={tooltip}
					>
						<div style={this.styles.warning}>{row.warningCount || ''}</div>
					</OverlayTrigger>
				</div>
			);
		}
